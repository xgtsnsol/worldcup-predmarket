...
pub struct Claim<'info> {
    ...
    #[account(
        ...
        bump = escrow.bump,
        constraint = escrow.state == EscrowState::Settled @ crate::errors::SettlementError::InvalidEscrowState,
    )]
...
pub fn handler(ctx: Context<Claim>) -> Result<()> {
    let is_winner = if ctx.accounts.escrow.depositor_won {
        ctx.accounts.claimant.key() == ctx.accounts.escrow.depositor
    } else {
        ctx.accounts.claimant.key() == ctx.accounts.escrow.recipient
    };
    require!(is_winner, crate::errors::SettlementError::NotClaimant);
